...
tm_expiring_contracts <- function(country_name, contract_end_year, league_url = NA) {
  ...
  if (is.na(league_url)) {
    ...
    tryCatch(
      {
        meta_df_seasons <- meta_df %>%
          dplyr::filter(.data[["country"]] %in% country_name)
      },
      error = function(e) {
    ...
    comp_url <- meta_df_seasons$comp_url %>%
      unique() %>%
      .[1]
  } else {
    tryCatch(
  ...
  expiring_urls <- expiring_page %>%
    rvest::html_nodes(".tm-pagination__list-item a") %>%
  ...
  get_each_page <- function(page_url) {
    if (length(expiring_pages) == 0) {
      ...
      exp_pg <- xml2::read_html(page_url) %>%
    ...
    out_df <- cbind(
      player_name, player_url, position, nationality, second_nationality,
      current_club, contract_expiry, contract_option, player_market_value, transfer_fee, agent
    ) %>% data.frame()

    if (is.na(league_url)) {
      out_df <- meta_df_seasons %>%
        dplyr::select(.data[["comp_name"]], .data[["country"]], .data[["comp_url"]]) %>%
        dplyr::distinct() %>%
        cbind(out_df)
    } else {
