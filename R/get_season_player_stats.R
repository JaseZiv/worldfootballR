...
#' }
fb_player_scouting_report <- function(player_url) {
  player_page <- xml2::read_html(player_url)

  player_name <- player_page %>% rvest::html_nodes("h1") %>% rvest::html_text() %>% stringr::str_squish()
  season <- player_page %>% rvest::html_nodes(".stats_pullout") %>% rvest::html_node("h4") %>% rvest::html_text()

  scouting_report <- player_page %>%
    rvest::html_nodes("#all_scout") %>%
    rvest::html_nodes(".table_container") %>%
    rvest::html_nodes("table") %>%
    rvest::html_table() %>% data.frame()

  scouting_report <- scouting_report %>%
    dplyr::filter(.data$Statistic != "")

  scouting_report <- cbind(player_name, season, scouting_report)
