...
```{r fotmob_get_league_matches-past}
...
You can extract momentum data with the `fotmob_get_match_momentum()` function. At time of writing, momentum is provided in two sets: (1) `"main"` ("Using xT SA-version") and (2) `"alternateModels"` ("Using xT SA-version without rolling").

```{r fotmob_get_match_momentum}
momentum <- fotmob_get_match_momentum(3901251)
dplyr::glimpse(momentum)
```

Note that matches before the 2022/23 season (for European leagues) do not have momentum available.
